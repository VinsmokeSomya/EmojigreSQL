...
FOR i IN 1.._tests LOOP
  ...
  encoded := emojigresql.encode(random_bytes);
  decoded := emojigresql.decode(encoded);
  IF decoded = random_bytes THEN
    _ok := _ok + 1;
  ELSE
    RAISE WARNING '% % -> % -> %', i, random_bytes, encoded, decoded;
  END IF;
  encoded := emojigresql.from_text(random_bytes::text);
  decoded_text := emojigresql.to_text(encoded);
